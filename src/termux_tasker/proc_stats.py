...
@dataclass(frozen=True)
class TreeStats:
    """Aggregate RSS over a root PID plus all its recursive descendants."""
...
def system_summary(proc_root: Path = _PROC_ROOT) -> SystemSummary:
    """Snapshot CPU percent and memory (best-effort).
...
def cpu_percent_delta(key: str, cpu_seconds: float, now: float | None = None) -> float | None:
    ...
    Feed the same ``key`` with a fresh counter reading on every tick; returns
    the busy % over the interval since the previous call, or None when there
    is no usable previous sample (first call, clock issue, or the counter
    went backwards because PIDs were recycled).
    """
...
def cpu_percent_total(
    ...
    overrides ``os.cpu_count()`` (used by tests for determinism). None on
    the first call or when nothing is readable.
    """
...
def proc_stat(pid: int, proc_root: Path = _PROC_ROOT) -> ProcStat | None:
    """Snapshot one PID's name and RSS; None when gone or hidden."""
...
def tree_stats(roots: list[int], proc_root: Path = _PROC_ROOT) -> TreeStats:
    """Aggregate RSS over root PIDs plus all recursive descendants."""
    if not roots:
        return TreeStats(pids=(), num_procs=0, rss_total=0)
...
def format_bytes(num: int | None) -> str:
    """Format a byte count with the most suitable unit; n/a when unknown."""
    if num is None:
        return "n/a"
    if num < 0:
        return "n/a"
    ...
    for unit in ("B", "KiB", "MiB", "GiB", "TiB"):
        ...
        value /= 1024
    return f"{value:.1f} TiB"  # unreachable, keeps the happy path last
