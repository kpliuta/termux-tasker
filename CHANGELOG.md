...
- Per-runner CPU % no longer spikes into the hundreds of percent when short-lived children churn between ticks: deltas are computed per PID (newborns baselined, exited PIDs ignored) in a single-walk `tree_report()`, which also reports the hottest child shown as `top <name> <pct>%` on the runner line.
